...
func ChangeColor(color Color) (string, error) {
	if color == Blue {
		...
		// workstation-01
		rs1 := mqtt.BrokerClient.Publish("homeassistant/0x00158d00038ea806/set", 1, false, file)
		rs1.Wait()
		// workstation-02
		rs2 := mqtt.BrokerClient.Publish("homeassistant/0x00158d00038ebb56/set", 1, false, file)
		rs2.Wait()
		// workstation-03
		rs3 := mqtt.BrokerClient.Publish("homeassistant/0x00158d00038eca7c/set", 1, false, file)
		rs3.Wait()
